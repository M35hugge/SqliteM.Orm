...
namespace Tests.Tests
{
    ...
    public class SQLiteMClientTests
    {
        ...
        ServiceProvider sp = TestHost.CreateProvider(out dbPath);
        SQLiteMClient client = new(dbPath);
        ...
        public async Task ClientQueryAsync()
        {
            ...
            var id = await client.InsertAsync(person1);
            var bob = await client.QueryAsync<Person>(Query.WhereEquals("FirstName", person1.FirstName));

            Assert.NotNull(bob);
            Assert.Single(bob);
            Assert.Equal(bob.First().FirstName, person1.FirstName);
