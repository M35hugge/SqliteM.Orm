...
internal class UnitOfWorkFactory : IUnitOfWorkFactory
{
    ...
    /// <param name="pragmaOptions">
    /// PRAGMA-Konfiguration, die beim Öffnen jeder Verbindung angewendet wird.
    /// Wenn <c>null</c>, werden <see cref="PragmaOptions.Default"/> verwendet.
    /// </param>
    /// <exception cref="ArgumentNullException"><paramref name="factory"/> ist <c>null</c>.</exception>
    public UnitOfWorkFactory(IConnectionFactory factory, PragmaOptions? pragmaOptions = null)
    {
        ArgumentNullException.ThrowIfNull(factory);
        _factory = factory;
        _pragmaOptions = pragmaOptions ?? PragmaOptions.Default;
    }

    /// <inheritdoc />
    /// <remarks>
    /// Die erzeugte <see cref="UnitOfWork"/> öffnet die Verbindung, wendet die
    /// konfigurierten PRAGMAs an (z. B. <c>foreign_keys = ON</c>, <c>journal_mode = WAL</c>)
    /// und startet eine Transaktion.
    /// </remarks>
    public Task<IUnitOfWork> CreateAsync(CancellationToken ct = default)
    {
        return Task.FromResult<IUnitOfWork>(new UnitOfWork(_factory, _pragmaOptions));
    }
}
