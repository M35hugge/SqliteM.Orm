...
- xUnit-Tests unter `SQLiteM.Tests`
- Für Integrations-DB-Tests: Dateidatenbank pro Testlauf oder Shared In-Memory mit Keeper-Connection
...
NuGet-Publish (mit API-Key):

```bash
dotnet nuget push .\**\bin\Release\*.nupkg  -k <NUGET_API_KEY> -s https://api.nuget.org/v3/index.json
dotnet nuget push .\**\bin\Release\*.snupkg -k <NUGET_API_KEY> -s https://api.nuget.org/v3/index.json
```

## Richtlinien / Qualität

- `<Nullable>enable</Nullable>` und (optional) `TreatWarningsAsErrors` im Release
- Öffentliche API minimal (Abstraktionen public, Implementierungen intern)
- XML-Doku auf der öffentlichen API
- Parametrisierte SQLs (keine String-Konkatenation in WHERE)
- UnitOfWork nicht nach Commit weiterverwenden
